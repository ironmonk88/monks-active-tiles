...
export class MonksActiveTiles {
    ...
    static async getEntities(args, defaultType, entry) {
        ...
        if (id == 'tile')
            ...
            entities = action.action == "create" ?
                game.users.map(u => !u.isGM && u.character).filter(t => !!t)
                : tile.parent.tokens.filter(t => {
    ...
    static async init() {
        ...
        document.body.addEventListener("mousemove", function () {
            ...
            for (let tile of canvas.scene.tiles) {
                ...
                if (!triggerData || !triggerData.active || !(triggers.includes("hoverin") || triggers.includes("hoverout") || triggerData.pointer))
                    continue;
        ...
        let retrievePlaceablesWithXY = function(entity, placeablesToCheck) {
            ...
            const placeableContains = function(placeable, position) {
                ...
                const result0 = Number.between(position.x, x, x + w) && Number.between(position.y, y, y + h);

                // const result1 = placeable.x + placeable.w > position.x &&
				// placeable.y + placeable.h > position.y &&
				// placeable.x < position.x &&
				// placeable.y < position.y;

                // const result =  position.x > placeable.x &&
				// position.y > placeable.y &&
				// position.x < placeable.x + placeable.w &&
				// position.y < placeable.y + placeable.h;

                return result0;
            }
            const clickPlaceables = placeablesToCheck.filter((placeable) => placeableContains(placeable, position)) || [];
            return clickPlaceables;
        }
        ...
        let leftClickOnTokenLayer = async function (wrapped, ...args) {
            if(setting("allow-note")) {
                ...
                args[0].type = "leftclick";
                noteControl(notes[0], wrapped, ...args);
            }
            return wrapped(...args);
        }

        let rightClickOnTokenLayer = async function (wrapped, ...args) {
            if(setting("allow-note")) {
                const notes = retrievePlaceablesWithXY(this,canvas?.notes?.placeables);
                args[0].type = "rightclick";
                noteControl(notes[0], wrapped, ...args);
            }
            return wrapped(...args);
        }
        ...
        if (game.modules.get("lib-wrapper")?.active) {
            libWrapper.register("monks-active-tiles", "TokenLayer.prototype._onClickLeft", leftClickOnTokenLayer, "WRAPPER");
        } else {
            const oldTokenLayer = TokenLayer.prototype._onClickLeft;
            TokenLayer.prototype._onClickLeft = function (event) {
                return leftClickOnTokenLayer.call(this, oldTokenLayer.bind(this), ...arguments);
            }
        }

        if (game.modules.get("lib-wrapper")?.active) {
            libWrapper.register("monks-active-tiles", "TokenLayer.prototype._onClickRight", rightClickOnTokenLayer, "WRAPPER");
        } else {
            const oldTokenLayer = TokenLayer.prototype._onClickRight;
            TokenLayer.prototype._onClickRight = function (event) {
                return rightClickOnTokenLayer.call(this, oldTokenLayer.bind(this), ...arguments);
            }
        }
                ...
                if ((data.userid == undefined || data.userid.find(u => u == game.user.id) != undefined) && (data.sceneid == undefined || canvas.scene.id == data.sceneid)) {
                    ...
                    if (tile) {
                        ...
                        AudioHelper.play({ src: data.src, volume: (data.fade > 0 ? 0 : volume), loop: data.loop }, false).then((sound) => {
                            if (data.fade > 0)
                                sound.fade(volume * game.settings.get("core", "globalAmbientVolume"), { duration: data.fade * 1000 });
    ...
    static setupTile() {
        TileDocument.prototype.pointWithin = function (point) {
            ...
            */
            ...
            return triggerData.usealpha ? this._object?._texturePolygon.contains(pt.x, pt.y) : true;
        }

        TileDocument.prototype.tokensWithin = function () {
            return this.parent.tokens.filter(t => {
                const midToken = {
                    x: t.x + ((Math.abs(t.width) * t.parent.dimensions.size) / 2),
                    y: t.y + ((Math.abs(t.height) * t.parent.dimensions.size) / 2),
                };
        ...
        TileDocument.prototype.checkClick = function (pt, clicktype = 'click') {
            let triggerData = this.flags["monks-active-tiles"];
            let triggers = MonksActiveTiles.getTrigger(triggerData?.trigger);
            if (triggerData && triggerData.active && triggers.includes(clicktype)) {
                ...
                let doTrigger = async function (idx) {
                    ...
                    if (stop.stop) {
                        ...
                        if (game.modules.get("drag-ruler")?.active) {
                            ...
                            options.animate = false;
                            await document.update({ x: endPt.x, y: endPt.y }, { bypass: true });
                        } else {
                            update.x = endPt.x;
                            update.y = endPt.y;
                            //options.bypass = true;
        ...
        for (let tile of combat.scene.tiles) {
            ...
            if (triggerData && triggerData.active && triggerData.actions?.length > 0) {
                ...
                if ((delta.turn || delta.round) && triggers.includes('turn')) {
                    let tokens = [combat.combatant.token];
                    tile.trigger({ tokens: tokens, method: 'trun' });
                }
...
Hooks.on("deleteCombat", async function (combat, delta) {
    if (combat.started && game.user.isGM && combat.scene) {
        for (let tile of combat.scene.tiles) {
            let triggerData = tile.flags["monks-active-tiles"];
            let triggers = MonksActiveTiles.getTrigger(triggerData?.trigger);
            if (triggerData && triggerData.active && triggerData.actions?.length > 0 && triggers.includes('combatend')) {
                let tokens = combat.combatants.map(c => c.token);
...
Hooks.on("canvasReady", () => {
    ...
    for (let tile of canvas.scene.tiles) {
        let triggerData = tile.flags["monks-active-tiles"];
        let triggers = MonksActiveTiles.getTrigger(triggerData?.trigger);
        if (triggerData && triggerData.active && triggers.includes("ready")) {
            //check to see if this trigger is restricted by control type
            if ((triggerData.controlled == 'gm' && !game.user.isGM) || (triggerData.controlled == 'player' && game.user.isGM))
                return;
