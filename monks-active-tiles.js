...
export class MonksActiveTiles {
    ...
    static triggerActions = {
        ...
        'pancanvas': {
            ...
            fn: async (args = {}) => {
                ...
                let dest = await MonksActiveTiles.getLocation.call(tile, action.data.location, value, null, userid);
            ...
            fn: async (args = {}) => {
                ...
                if (entities && entities.length > 0) {
                    ...
                    for (let entity of entities) {
                        ...
                        let newPos = {
                            x: dest.x - (action.data?.location?.id == "origin" ? 0 : ((object.w || object.width) / 2)),
                            y: dest.y - (action.data?.location?.id == "origin" ? 0 : ((object.h || object.height) / 2))
                        };
        ...
        'create': {
            ...
            ctrls: [
                {
                    ...
                    restrict: (entity) => { return (entity instanceof Actor || entity instanceof JournalEntry); },
                    required: true,
                    defaultType: 'actors',
            ...
            fn: async (args = {}) => {
                ...
                if (entities && entities.length > 0) {
                    const actors = await entities.reduce(async (promise, e) => {
                        let a = await promise;
                        if (e instanceof JournalEntry) {
                            if ((e.data.flags["monks-enhanced-journal"]?.actors || []).length && game.modules.get("monks-enhanced-journal")?.active) {
                                for (let ea of (e.data.flags['monks-enhanced-journal']?.actors || [])) {
                                    let actor;
                                    if (ea.pack) {
                            ...
                            } else if (e.data.flags["quick-encounters"]?.quickEncounter && game.modules.get("quick-encounters")?.active) {
                                try {
                                    let data = JSON.parse(e.data.flags["quick-encounters"]?.quickEncounter);
                                    ...
                                    for (let ea of (data.extractedActors || [])) {
                                        ...
                                        if (actor) {
                                            for (let i = 0; i < (ea.numActors || 1); i++) {
                                                a.push(actor);
                                            }
                                        }
                                    }
                    ...
                    for (let actor of actors) {
                        ...
                        if (action.data.avoidtokens) {
                            data = mergeObject(data, MonksActiveTiles.findVacantSpot(data, { data: td }, tile.parent, dest, action.data.snap));
                            td.x = data.x;
                            td.y = data.y;
                        }
        ...
        'playsound': {
            ...
            fn: async (args = {}) => {
                ...
                volume = Math.clamped(volume, 0, 1);
        ...
        'trigger': {
            ...
            fn: async (args = {}) => {
                const { tile, userid, action } = args;
                let entities = await MonksActiveTiles.getEntities(args, null, 'tiles');
                if (entities.length == 0)
                    return;

                let tokens = await MonksActiveTiles.getEntities(args, (action.data?.token?.id || "previous"), "tokens");
                //if (tokens.length == 0)
                //    return;
                ...
                for (let entity of entities) {
                    ...
                    let newargs = Object.assign({}, args, {tokens: tokens, tile: entity, method: "Trigger"});
        ...
        'addtocombat': {
            ...
            fn: async (args = {}) => {
                ...
                if (action.data.addto == "remove") {
                    let combatants = entities.filter(t => t.inCombat).map(t => { return combat.getCombatantByToken(t.id); });
        ...
        'imagecycleset': {
                ...
                if (entities && entities.length > 0) {
                    for (let entity of entities) {
                        ...
                        if (act) {
                            ...
                            act.data.imgat = (Math.clamped((action.data?.imgat || 1), 1, files.length) % files.length);
            ...
            content: async (trigger, action) => {
                let entityName = await MonksActiveTiles.entityName(action.data?.entity, 'tiles');
                return `<span class="action-style">${i18n(trigger.name)}</span> to <span class="details-style">"${action.data.imagat}"</span> for <span class="entity-style">${entityName}</span>`;
            }
        },
        'delete': {
        ...
        'dialog': {
            ...
            content: async (trigger, action) => {
                let msg = (action.data.content.length <= 15 ? action.data.content : action.data.content.substr(0, 15) + "...");
        ...
        'exists': {
            ...
            fn: async (args = {}) => {
                let { action } = args;
                let entities = await MonksActiveTiles.getEntities(args, null, action.data?.collection || "tokens");
            ...
            content: async (trigger, action) => {
                ...
                let goto = action.data?.none || "";
                let count = action.data?.count ?? "> 0";
                return `<span class="filter-style">Continue if</span> <span class="entity-style">${entityName}</span> <span class="value-style">"${count}"</span>${goto != "" ? ' goto <span class="details-style">"' + goto + '"</span> if none exist' : ""}`;
            }
        },
        'triggercount': {
            ...
            fn: async (args = {}) => {
                ...
                let cando = false;
                try {
                    cando = !!eval(tile.countTriggered() + " " + count);
                } catch {
                }
            ...
            content: async (trigger, action) => {
                let goto = action.data?.none || "";
                let count = action.data?.count ?? "> 0";
                return `<span class="filter-style">Continue if</span> Tile triggered <span class="value-style">"${count}"</span>${goto != "" ? ` goto <span class="details-style">"${goto}"</span> if it hasn't` : ""}`;
            }
        },
        'tokencount': {
        ...
        'inventory': {
            ...
            fn: async (args = {}) => {
                ...
                let result = entities.filter(entity => {
                    if (!entity.actor)
                        return false;
                    return entity.actor.items.find(i => i.name == action.data.item) != undefined;
                });
            ...
            content: async (trigger, action) => {
                let entityName = await MonksActiveTiles.entityName(action.data?.entity, action.data?.collection);
                return `<span class="filter-style">Find</span> <span class="entity-style">${entityName}</span> with item <span class="value-style">&lt;${action.data?.item}&gt;</span>`;
            }
        },
        'anchor': {
    ...
    static async getLocation(location, value, pt, userid) {
        ...
        if (location.id == 'previous')
            ...
            if (user && user.character?.id) {
                ...
                if (scene) {
                    ...
                    if (token) {
                        return {
                            x: token.data.x + ((token.data.width * scene.dimensions.size) / 2),
                            y: token.data.y + ((token.data.height * scene.dimensions.size) / 2),
                            scene: scene.id
                        };
                    }
                }
    ...
    static async _showDialog(tile, token, value, type, title, content, options, yes, no) {
        ...
        if (type == 'confirm') {
            return Dialog.confirm({
                ...
                content: content,
                yes: () => {
                    return (yes ? { goto: yes } : { continue: false });
                },
                no: () => {
                    return (no ? { goto: no } : { continue: false });
                },
                options: opts,
                rejectClose: true
        ...
        TileDocument.prototype.checkClick = function (pt, clicktype = 'click') {
            ...
            if (triggerData && triggerData.active && triggerData.trigger == clicktype) {
                ...
                if (pt == undefined || (triggerData.usealpha ? this.object.containsPixel(pt.x, pt.y) : this.pointWithin(pt))) {
                    //this.preloadScene();
                    return this.trigger({ tokens: tokens, method: (clicktype == 'dblclick' ? 'DoubleClick' : 'Click'), pt: pt });
                }
            }
        }
...
*/
                                ...
                                window.setTimeout(function () {
                                    ...
                                    tile.document.trigger({ tokens: [document], method: triggerPt.method, pt: pt, options: { original } });
        ...
        for (let layer of [canvas.background.tiles, canvas.foreground.tiles]) {
            for (let tile of layer) {
                ...
                if (triggerData && triggerData.active && triggerData.actions.length > 0 &&
                    ...
                    tile.document.trigger({ tokens: tokens, method: (triggerData.trigger == 'round' ? 'Round' : (triggerData.trigger == 'combatstart' ? 'Combat Start' : 'Turn')) });
                }
            }
        }
    }
        ...
        for (let layer of [canvas.background.tiles, canvas.foreground.tiles]) {
            for (let tile of layer) {
                ...
                if (triggerData && triggerData.active && triggerData.actions.length > 0 && triggerData.trigger == 'combatend') {
                    ...
                    tile.document.trigger({ tokens: tokens, method: 'Combat End' });
                }
            }
        }
    }
